"""
This the main module for this case study. This is used to create/ register the Data Models & REST APIS which are required to support this application.
"""
...
# Define constants
DB_URL = r"sqlite:///case_study.db" # DB URL where application data will be stored
WX_DATA_PATH = r"data\wx_data\*" # Path of the weather data from where to load and ingest into DB
YLD_DATA_PATH = r"data\yld_data\*" # Path of the yield data from where to load and ingest into DB
PER_PAGE = 10 # per page max number of records return the GET API
BATCH_SIZE = 100000 # this denotes after how many record, a commit will be issued while ingesting the data in DB

# initialize the flask app
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = DB_URL

# initialize the DB object. This DB object will be used to interact with the actual DB.
db = SQLAlchemy(app)

# initialize the marshmallow object. This allows to easy serialization/deserialization of Data Model objects
ma = Marshmallow(app)
...
class WXData(db.Model):
    """
    This class represent the wxdata table in DB to store the weather data.
    """
    ...
    def __repr__(self):
        return '<WXData %s %s %s %s %s>' % (
            self.station_name, self.date, self.max_temp, self.min_temp, self.precipitation)
...
class YLDData(db.Model):
    """
    This class represent the ylddata table in DB to store the yield data.
    """
...
class WXStats(db.Model):
    """
    This class represent the wxstats table in DB to store the weather stats data.
    """
    station_name = db.Column(db.String, primary_key=True)
    year = db.Column(db.Integer, primary_key=True)
    ...
    def __repr__(self):
        return '<WXStats %s %s %s %s %s>' % (
            self.station_name, self.year, self.avg_max_temp, self.avg_min_temp, self.total_precipitation)
...
def init_command():
    ...
    # check if DB already exist of not. Only re-create DB if do not exist
...
def load_wx_data(wx_data_path, existing_wx_data):
    """
    This function load the data from all files present in wx_data_path
    parse the required fields for each line and compare with existing data in weather table in DB
    Only append to the records list if already not exist in table. This will ensure running the ingest api multiple times
    will not create duplicate rows in table.
    Note: For checking whether record exist in table or not, only station name is compared.
    """
    records = []
...
def load_yld_data(yld_data_path, existing_yld_data):
    """
    This function load the data from all files present in yld_data_path
    parse the required fields for each line and compare with existing data in yield table in DB
    Only append to the records list if already not exist in table. This will ensure running the ingest api multiple times
    will not create duplicate rows in table.
    """
    records = []
...
class IngestData(Resource):
    """
    This function is invoked when POST /api/ingest API is hit.
    This load the weather and yield data from respective paths, 
    checks for if record already exist in table or not (with the help of helper functions load_wx_data & 
    load_yld_data and finally ingest the weather and yield data in respective table
    """
    def post(self):
        app.logger.info('Data Ingestion API Invoked')

        existing_wx_data = {item['station_name'] for item in
                            wxdatas_schema.dump(WXData.query.with_entities(WXData.station_name).all())}
        existing_yld_data = ylddatas_schema.dump(YLDData.query.all())

        app.logger.info('{} records already exist for Weather Data'.format(
            len(wxdatas_schema.dump(WXData.query.with_entities(WXData.station_name).all()))))
...
class CalculateStats(Resource):
    """
    This function is invoked when POST /api/cal_stats API is hit.
    This calculates the stats corresponding to weather data and overwrite the stats in wxstats table in DB
    """
    def post(self):
        ...
        new_wx_stats = [WXStats(station_name=stat['station_name'], year=stat['year'], avg_max_temp=stat['avg_max_temp'],
                                avg_min_temp=stat['avg_min_temp'], total_precipitation=stat['total_precipitation']) for
                        stat in wxstats_schema.dump(new_stats)]
...
class Weather(Resource):
    """
    This function is invoked when GET /api/weather API is hit.
    This query the wxdata table in DB and return the json formatted response based on the query parameters
    """
    def get(self):
        ...
        if station_name is None and date is None:
            ...
            wx_data = WXData.query.filter(WXData.date == datetime.datetime.strptime(date, '%Y%m%d')).\
            ...
            wx_data = WXData.query.\
                filter(db.and_(WXData.station_name == station_name,
                               WXData.date == datetime.datetime.strptime(date, '%Y%m%d')))\
                .paginate(page, PER_PAGE, error_out=False)
...
class Yield(Resource):
    """
    This function is invoked when GET /api/yield API is hit.
    This query the ylddata table in DB and return the json formatted response based on the query parameters
    """
    def get(self):
...
class WeatherStats(Resource):
    """
    This function is invoked when GET /api/weather/stats API is hit.
    This query the wxstats table in DB and return the json formatted response based on the query parameters
    """
    def get(self):
        ...
        if station_name is None and year is None:
            ...
            wx_stats = WXStats.query.filter(WXStats.station_name == station_name).paginate(page, PER_PAGE,
                                                                                           error_out=False)

        else:
            wx_stats = WXStats.query.filter(
                db.and_(WXStats.station_name == station_name, WXStats.year == year)).paginate(page, PER_PAGE,
                                                                                              error_out=False)
